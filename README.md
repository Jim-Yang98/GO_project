# 爬蟲專案與聲量分析
針對手遊產品建置一站式輿情監測流程。開發具容錯與抗阻擋能力的跨平台爬蟲，結合深度學習模型進行情緒標籤預測，並透過 Power BI 建立儀表板，將社群留言轉化為可輔助決策的量化指標。
本專案以Pokemon GO為例，針對批踢踢 (PTT)、巴哈姆特與 Dcard 之2025版面進行自動化爬蟲，並透過 BERT 模型進行文本標籤與聲量分析。
